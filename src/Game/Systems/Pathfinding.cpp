...
std::vector<std::pair<int, int>> Pathfinding::getNeighbors(int row, int col, Grid* grid) {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        if (isCellWalkable(newRow, newCol, grid)) {
            // para movimientos diagonales, verificar que no estamos cortando esquinas
            if (dr[i] != 0 && dc[i] != 0) {  // es diagonal
                // verificar que los dos lados adyacentes tambien son transitables
                if (isCellWalkable(row + dr[i], col, grid) &&
                    isCellWalkable(row, col + dc[i], grid)) {
                    neighbors.push_back({newRow, newCol});
                }
            } else {
                // movimiento recto, siempre valido si la celda es transitable
                neighbors.push_back({newRow, newCol});
            }
        }
    }
...
std::vector<sf::Vector2f> Pathfinding::findPath(Grid* grid, const sf::Vector2f& worldStart, const sf::Vector2f& worldGoal) {
    ...
    while (!openList.empty()) {
        ...
        for (const auto& [neighborRow, neighborCol] : neighbors) {
            ...
            float moveCost = (std::abs(neighborRow - current->row) + std::abs(neighborCol - current->col)) == 2 ? 1.414f : 1.0f;
